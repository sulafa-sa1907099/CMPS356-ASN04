...
export default function IdeaCard({ user }) {
    const port = env.port
    const ideas = useQuery(['ideas', user], () => axios.get(`sulafa1907099-asn04-39ar740ud-sulafa-sa1907099.now.sh/${user}/ideas`))
    const queryClient = useQueryClient()

    const ideasDeleteMutation = useMutation((id) => axios.delete(`sulafa1907099-asn04-39ar740ud-sulafa-sa1907099.now.sh/${user}/ideas?id=${id}`),
        {
            onSuccess: () => {
